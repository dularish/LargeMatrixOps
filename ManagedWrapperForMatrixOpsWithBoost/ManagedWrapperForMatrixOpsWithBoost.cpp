	...
	if (nativeMatrixManager) {
		...
		if (matrixDict.find(matrixNameAUnmanaged) == matrixDict.end()) {
			...
			if (matrixAPtr->size2() == matrixBPtr->size1()) {
				this->InstantiateMatrix(labelForMatrixAB, matrixAPtr->size1(), matrixBPtr->size2());
				MatrixPtr matrixABPtr = matrixDict[matrixNameABUnmanaged];
				matrix<double> prodMatrix = matrix<double>(matrixAPtr->size1(), matrixBPtr->size2(), 0);
				prodMatrix = prod(*matrixAPtr, *matrixBPtr);//Working
				//(*matrixABPtr) = prod(*matrixAPtr, *matrixBPtr); //But this working is needed
				cout << "product matrix :" << prodMatrix << endl;
			}
			else {
